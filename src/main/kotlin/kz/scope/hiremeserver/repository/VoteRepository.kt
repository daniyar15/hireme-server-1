...
import kz.scope.hiremeserver.model.ChoiceVoteCount
import kz.scope.hiremeserver.model.Vote
...
interface VoteRepository : JpaRepository<Vote, Long> {
    @Query("SELECT NEW kz.scope.hiremeserver.model.ChoiceVoteCount(v.choice.id, count(v.id)) FROM Vote v WHERE v.poll.id in :pollIds GROUP BY v.choice.id")
    fun countByPollIdInGroupByChoiceId(@Param("pollIds") pollIds: List<Long>): List<ChoiceVoteCount>

    @Query("SELECT NEW kz.scope.hiremeserver.model.ChoiceVoteCount(v.choice.id, count(v.id)) FROM Vote v WHERE v.poll.id = :pollId GROUP BY v.choice.id")
